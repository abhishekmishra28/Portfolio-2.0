      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden bg-background/98 backdrop-blur-xl border-b border-border shadow-xl"
          >
            <div className="container-custom py-6 space-y-1 max-h-[calc(100vh-4rem)] overflow-y-auto">
              {navItems.map((item, index) => (
                <motion.button
                  key={item.name}
                  onClick={() => handleNavClick(item.href)}
                  ...
                  className="block w-full text-left px-4 py-3 text-base font-medium text-muted-foreground hover:text-primary hover:bg-secondary/80 rounded-lg transition-all active:scale-[0.98]"
                >
                  {item.name}
                </motion.button>
              ))}
